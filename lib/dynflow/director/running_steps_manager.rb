      ...
      def done(step)
        ...
        if step.state == :suspended
          ...
          while (work = @work_items.shift(step.id))
            message = "step #{step.execution_plan_id}:#{step.id} dropping event #{work.request_id}/#{work.event}"
            @world.logger.warn message
            finish_event_result(work) do |f|
              f.reject UnprocessableEvent.new(message).tap { |e| e.set_backtrace(caller) }
            end
          end
          raise 'assert' unless @work_items.empty?(step.id)
